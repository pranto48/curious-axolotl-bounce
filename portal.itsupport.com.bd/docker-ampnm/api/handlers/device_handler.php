...
switch ($action) {
    case 'import_devices':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            // License check for device creation limit
            if ($_SESSION['license_status'] !== 'active' && $_SESSION['license_status'] !== 'free') {
                http_response_code(403);
                echo json_encode(['error' => 'License is not active. Cannot import devices.']);
                exit;
            }

            $devices_to_import = $input['devices'] ?? [];
            if (empty($devices_to_import) || !is_array($devices_to_import)) {
            ...
            try {
                $stmt_count = $pdo->prepare("SELECT COUNT(*) FROM `devices` WHERE user_id = ?");
                $stmt_count->execute([$current_user_id]);
                $current_device_count = $stmt_count->fetchColumn();
                $max_devices = $_SESSION['license_max_devices'] ?? 0;

                if (($current_device_count + count($devices_to_import)) > $max_devices) {
                    $pdo->rollBack();
                    http_response_code(403);
                    echo json_encode(['error' => "License limit reached. You can only import " . ($max_devices - $current_device_count) . " more devices."]);
                    exit;
                }
                ...
                foreach ($devices_to_import as $device) {
                    $stmt->execute([
                        $current_user_id,
    ...
    case 'check_all_devices_globally':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($_SESSION['license_status'] !== 'active' && $_SESSION['license_status'] !== 'free') {
                http_response_code(403);
                echo json_encode(['error' => 'License is not active. Cannot perform global device check.']);
                exit;
            }
    ...
    case 'ping_all_devices':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($_SESSION['license_status'] !== 'active' && $_SESSION['license_status'] !== 'free') {
                http_response_code(403);
                echo json_encode(['error' => 'License is not active. Cannot perform bulk device ping.']);
                exit;
            }
    ...
    case 'check_device':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($_SESSION['license_status'] !== 'active' && $_SESSION['license_status'] !== 'free') {
                http_response_code(403);
                echo json_encode(['error' => 'License is not active. Cannot check device status.']);
                exit;
            }
    ...
    case 'create_device':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($_SESSION['license_status'] !== 'active' && $_SESSION['license_status'] !== 'free') {
                http_response_code(403);
                echo json_encode(['error' => 'License is not active. Cannot create new devices.']);
                exit;
            }
            ...
            if ($current_device_count >= $max_devices) {
                http_response_code(403);
                echo json_encode(['error' => 'License limit reached. You cannot add more devices. Max devices: ' . $max_devices]);
                exit;
            }
    ...
    case 'update_device':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($_SESSION['license_status'] !== 'active' && $_SESSION['license_status'] !== 'free') {
                http_response_code(403);
                echo json_encode(['error' => 'License is not active. Cannot update devices.']);
                exit;
            }
    ...
    case 'delete_device':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($_SESSION['license_status'] !== 'active' && $_SESSION['license_status'] !== 'free') {
                http_response_code(403);
                echo json_encode(['error' => 'License is not active. Cannot delete devices.']);
                exit;
            }
    ...
    case 'upload_device_icon':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($_SESSION['license_status'] !== 'active' && $_SESSION['license_status'] !== 'free') {
                http_response_code(403);
                echo json_encode(['error' => 'License is not active. Cannot upload device icon.']);
                exit;
            }
