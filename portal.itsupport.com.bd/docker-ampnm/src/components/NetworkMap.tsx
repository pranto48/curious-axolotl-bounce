...
const NetworkMap = ({ devices, onMapUpdate, isLicensed, maxDevices, currentDeviceCount }: NetworkMapProps) => {
  ...
  const handleAddDevice = () => {
    if (!isLicensed) {
      showError('License required to add devices.');
      return;
    }
    if (maxDevices > 0 && currentDeviceCount >= maxDevices) {
      showError(`License limit reached. You can only add up to ${maxDevices} devices.`);
      return;
    }
  ...
  const handleDelete = async (deviceId: string) => {
    if (window.confirm('Are you sure you want to delete this device?')) {
      ...
      try {
        ...
        onMapUpdate(); // Trigger a map update to refresh device count for license check
      } catch (error) {
  ...
  const handleSaveDevice = async (deviceData: Omit<NetworkDevice, 'id' | 'position_x' | 'position_y'>, maxDevices: number, currentDeviceCount: number) => {
    try {
      if (editingDevice?.id) {
        ...
        // Add new device
        await addDevice({ ...deviceData, position_x: 100, position_y: 100, status: 'unknown' }, maxDevices, currentDeviceCount);
  ...
  const canAddDevice = isLicensed && (maxDevices === 0 || currentDeviceCount < maxDevices);
  ...
  return (
    <div style={{ height: '70vh', width: '100%' }} className="relative border rounded-lg bg-gray-900">
      ...
      <div className="absolute top-4 left-4 flex flex-wrap gap-2">
        <Button onClick={handleAddDevice} size="sm" disabled={!canAddDevice}>
