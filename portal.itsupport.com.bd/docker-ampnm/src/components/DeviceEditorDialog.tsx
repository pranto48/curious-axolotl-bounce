...
interface DeviceEditorDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (device: Omit<NetworkDevice, 'id' | 'position_x' | 'position_y'>, maxDevices: number, currentDeviceCount: number) => void;
...
export const DeviceEditorDialog = ({ isOpen, onClose, onSave, device, maxDevices, currentDeviceCount }: DeviceEditorDialogProps) => {
  ...
  const handleSubmit = (values: z.infer<typeof deviceSchema>) => {
    if (!device?.id && maxDevices > 0 && currentDeviceCount >= maxDevices) {
      showError(`License limit reached. You can only add up to ${maxDevices} devices.`);
      return;
    }
    onSave(values, maxDevices, currentDeviceCount);
    onClose();
  };
