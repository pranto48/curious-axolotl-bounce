...
export const addDevice = async (device: Omit<NetworkDevice, 'user_id'>, maxDevices: number, currentDeviceCount: number) => {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error('User not authenticated');

  if (maxDevices > 0 && currentDeviceCount >= maxDevices) {
    throw new Error(`License limit reached. You can only add up to ${maxDevices} devices.`);
  }
